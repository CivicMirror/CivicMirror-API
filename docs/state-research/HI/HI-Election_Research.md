...
| Stage 1 — Candidate Creation | ✅ Official CSV export verified | The Candidate Report provides a cycle-specific CSV export containing candidate, contest, party, contact, filing-date, and status fields. The portal is an ASP.NET/Telerik application; HTML paging remains a fallback. |
...
| Candidate Portal HAR | ✅ Captured | HARs verify page-size changes, numbered pagination, party and contest filters, election-cycle switching, and CSV/PDF exports. |
| Official Results API | ❌ Not identified | No official public Hawaii election-results REST or JSON API was verified. |
| Official Candidate API | ❌ No REST/JSON API identified | No documented candidate API was observed, but an undocumented full-cycle CSV form export is verified and should be the preferred ingestion path. |
...
**HAR captures reviewed:** August 5, 2026  
**Corrected:** August 6, 2026 — corrected the candidate-portal findings after a full HAR review; documented filters, cycle switching, and CSV/PDF exports; changed the recommended Stage 1 adapter from HTML-first to CSV-first; repaired Markdown structure and removed unresolved citation artifacts  
...
For Stage 1, election definitions come from Office of Elections proclamations and supplemental vacancy proclamations. The Candidate Report provides candidate filings and status through an undocumented but verified cycle-specific CSV export. Official preview ballots remain necessary to establish actual ballot appearance, precise contest wording, vote-for limits, party sections, and ballot-style applicability.

For Stage 2, the Office of Elections results index is the authoritative discovery page. It links certified statewide summaries, county reports, precinct detail, statements of vote, structured text files, and recount reports. The archive begins in 1992 and includes regular primary and general elections, special elections, vacancy elections, recounts, and ballot measures. As of the August 5, 2026 research capture, the index contained a 2026 section, but completed archived election results extended through 2024 because the 2026 primary had not yet occurred.

The main integration problem is an **adapter and normalization gap**, not a lack of official data:

1. Candidate ingestion should use the portal’s CSV form export first, with stateful HTML paging as a fallback.
2. Result ingestion requires a structured-text adapter plus historical HTML/PDF fallbacks.
...
4. Preview ballots are needed to validate ballot-qualified candidates and contest structure.
5. Different result eras use different formats and, in at least one case, different text schemas.

---

## Existing Research Inputs and Material Corrections

The previous repository research correctly identified the Office of Elections results page, certified PDFs, text files, the Hawaii Open Data portal, and historical coverage beginning in 1992. It described Stage 1 as dependent on Google Civic Information and Stage 2 as an unbuilt CKAN adapter.

Issue #156 correctly identified the native Candidate Report and requested investigation of pagination, changing `elid` values, exports, and browser-network behavior. The issue reported 411 records across 28 pages for the 2026 report.
...
- The Candidate Report has a working CSV export. The captured 2022 export contained all 509 candidate records and 12 columns.
- The CSV export is a form-driven download from the same `CandidateFiling.aspx` page, not a documented REST or JSON API.
- The captured CSV was requested while a contest filter was active, but it returned the full 2022 cycle. This suggests export may ignore the visible grid filter; confirm this behavior against the current cycle before relying on it as a permanent guarantee.
- Party and contest filter postback commands were captured.
- Election-cycle switching to `elid=92` and `elid=87` was captured. The portal updates its election-state cookie and serves the selected cycle.
- CSV and PDF export actions were captured successfully.
...
Repository and issue content should remain working research context. Official state sources and captured first-party network behavior remain the evidence for election facts and source behavior.
...
| Rank | Source | Responsible entity | Scope and subjects | Election types / cycles | Source type and access | Machine-readability / authentication | CivicMirror value and provenance |
|---:|---|---|---|---|---|---|---|
| 1 | Candidate Report — `https://olvr.hawaii.gov/Controls/CandidateFiling.aspx?elid=94` | Office of Elections | Contest, party, ballot name, legal name, contact fields, issued date, filed date, candidate status | Confirmed 2022, 2024, and 2026 reports | ASP.NET/Telerik HTML portal with form-driven CSV and PDF exports | Public, no login; maintain cookies and fresh ASP.NET hidden state for export or HTML actions | Preferred candidate source. Use CSV export first; retain HTML paging as fallback. |
| 2 | Election result text files — discovered at `https://elections.hawaii.gov/election-results/` | Office of Elections | Contest totals, choices, parties, registration, reporting status, voting-method totals, precinct and split information | Most regular primary/general elections from 2004 forward; coverage varies by year | Structured text / bulk download | Public, no login; schema varies by era | Preferred Stage 2 source. Discover URLs from the index rather than constructing them. |
| 3 | Campaign Spending Commission searchable datasets — `https://ags.hawaii.gov/campaign/cc/view-searchable-data/` | Hawaii Campaign Spending Commission | Contributions, expenditures, loans, other receipts, unpaid expenditures, committee filings | Candidate committees; searchable coverage described as 2015–2025, encompassing 2016–2024 elections, plus some amended earlier transactions | Socrata/SODA API; CSV, Excel, PDF; database portal | Public read/download access; an account is needed only to save custom views | Best verified official API, but it covers campaign finance rather than election administration. |
| 4 | Hawaii Open Data election datasets — `https://opendata.hawaii.gov/dataset/?organization=office-of-elections` | Office of Elections / Hawaii Open Data | Selected summary and precinct results | Confirmed mainly for 2010 and 2012 | Database portal; CSV, JSON, XML, RDF | Public structured downloads | Useful for selected historical validation, not the main results pipeline. |
| 5 | Proclamation archive — `https://elections.hawaii.gov/proclamations/` | Office of Elections and county clerks | Election dates, offices, districts, vacancy contests, voter-service details, measure categories | Current archive visibly includes 2020–2026 regular proclamations and selected vacancy/amendment notices | HTML page plus PDF | Public; HTML is readable, PDFs require document extraction | Preferred election-definition and amendment-monitoring source. |
| 6 | Contest Schedule — `https://elections.hawaii.gov/voting/contest-schedule/` | Office of Elections | Offices, seats, jurisdictions, districts, and terms by scheduled cycle | Current schedule includes 2026 and 2028 | HTML table | Public and scrapeable | Baseline office-cycle source; proclamations control for the actual election. |
| 7 | Official GIS files — `https://elections.hawaii.gov/resources/maps/geographic-information-systems-gis-files/` | Office of Elections / Hawaii State GIS Program | Election precincts, congressional districts, state Senate and House districts, census-block assignments | 2022 precincts, 2021 reapportionment, 2018 precincts, and linked boundary products | GIS service / geoportal / bulk download | Public; formats depend on linked layer or download | Preferred geography source. Record boundary vintage before joining to results. |
| 8 | Registration and Turnout Statistics — `https://elections.hawaii.gov/resources/registration-voter-turnout-statistics/` | Office of Elections | Registered voters, voters casting ballots, turnout percentages; recent district maps | Primary/general historical series beginning in 1959; recent maps through 2024 | HTML tables; ArcGIS maps | Public; HTML tables are machine-readable with cleanup | Strong reconciliation source; not a substitute for contest results. |
| 9 | Elected Officials — `https://elections.hawaii.gov/resources/elected-officials/` | Office of Elections | Current officeholders, terms, vacancies, and next scheduled elections | Current snapshot | HTML page | Public and scrapeable | Useful for incumbent and vacancy validation; retain access date because it changes over time. |
| 10 | Preview ballots — e.g. `https://elections.hawaii.gov/wp-content/uploads/2026-Primary-Ballots.pdf` | Office of Elections | Ballot styles, precinct codes, contest names, candidates, party sections, vote-for limits | Current-cycle primary/general ballots when published | PDF | Public but large and repetitive | Strong race and ballot-qualification validation source; expensive for routine extraction. |
| 11 | Certified result PDFs and recount PDFs — discovered from results index | Office of Elections | Human-readable summaries, county reports, precinct detail, statement of vote, recounts | 1992 forward; format and granularity vary | PDF | Public; older scans are machine-unfriendly | Human validation and historical fallback. PDFs are lowest-priority extraction inputs. |
| 12 | Certified historical HTML | Office of Elections | Statewide, county, and precinct results | Confirmed for 1996, 1998, and 2000, including the 1996 special election | HTML page / HTML scraping | Public and easier to extract than paired PDFs | Preferred over PDF for those historical years. |
...
|---|---:|---:|---|
| 2022 | 87 | 509 items / 34 pages; 509 records in captured CSV | `Issued`, `In Primary`, `In General` |
| 2024 | 92 | 395 items / 27 pages | `Issued`, `In Primary`, `In General`, `Elected` |
| 2026 | 94 | 411 items / 28 pages as of August 5, 2026 | `Issued`, `In Primary` |

The portal exposes page-size options of 10, 15, 20, and 50. Record counts are time-sensitive during active filing and status-update periods.

### Verified CSV fields

The captured 2022 CSV contains these 12 columns:
...
The HTML report presents the same information with display labels such as Contest, Party, Ballot Name, Legal Name, Mailing Address, Phone, Email, Website, Issued date, Filed date, and Status.
...
No stable public candidate ID was verified in the page, CSV, or HAR.

Use a source-scoped provisional key:

```text
HI|cycle|contest_raw|party_raw|legal_name|filing_date
```
...
2. Follow its current Candidate Report link.
3. Record the resolved URL and `elid`.
4. Store confirmed historical URLs for backfill.
5. Verify the returned cycle, dropdown value, and election-state cookie.
...
- Initial access is a GET to the `CandidateFiling.aspx` page.
- Candidate rows are returned in the full HTML response.
- Paging, page-size, filtering, cycle-selection, and export actions are form submissions.
- Request content type is `application/x-www-form-urlencoded`.
- Stateful actions include fresh `__VIEWSTATE`, `__EVENTVALIDATION`, and related ASP.NET state.
- Paging, page-size, and filtering responses return HTML.
- CSV export returns `text/csv; charset=utf-8` with a downloadable filename.
- PDF export returns `application/pdf` with a downloadable filename.
- No separate candidate REST, JSON, or XHR data endpoint was observed.
...
The request also included ASP.NET hidden state, filter fields, and Telerik client-state fields. The returned HTML initialized the page-size control with 20 selected.
...
The page HTML exposed visible mappings from page numbers to `javascript:__doPostBack(...)` targets. The observed control suffix increased by two, but an extractor should parse each response’s actual links instead of generating control names from the pattern.

### Verified filtering

The first HAR captured a party filter:

```text
__EVENTTARGET=ctl00$cphFooter$rdgSearch
__EVENTARGUMENT=FireCommand:ctl00$cphFooter$rdgSearch$ctl00;Filter;Party|?democratic|?Contains
```

The second HAR captured a contest filter:

```text
__EVENTTARGET=ctl00$cphFooter$rdgSearch
__EVENTARGUMENT=FireCommand:ctl00$cphFooter$rdgSearch$ctl00;Filter;Contests|?U.S. REPRESENTATIVE, DIST I|?Contains
```

The form also exposes fields for ballot-name and legal-name filters. Their field names are known, although separate HAR actions applying those two filters were not captured.

### Verified election-cycle switching

The first capture includes a dropdown postback with:

```text
__EVENTTARGET=ctl00$cphFooter$ddlElection
```

The response redirected to:

```text
/Controls/CandidateFiling.aspx?elid=92
```

The second capture includes direct access to `elid=87`, subsequent postbacks to the `elid=87` URL, `ctl00$cphFooter$ddlElection=87`, and the `HIVIP` cookie set to `elID=87`.

Cycle switching is therefore functional. A new session is not inherently required, but an implementation should verify the URL, selected dropdown value, cycle label, and `HIVIP` cookie after every switch. Invalid or stale `elid` values may redirect or fail and must not be interpreted as valid election data.

### Verified CSV export

The second HAR captured a successful CSV form submission using:

```text
ctl00$cphFooter$rdgSearch$ctl00$ctl02$ctl00$ExportToCsvButton=
ctl00$cphFooter$ddlElection=87
```
...
Content-Disposition: attachment; filename=CandidateFiling__202685_1056.csv
```

The captured CSV contained:

- 1 header row
- 509 candidate records
- 12 columns
- records spanning the full 2022 candidate report

A contest filter for `U.S. REPRESENTATIVE, DIST I` was active when the export was requested, but the resulting CSV included contests from `U.S. SENATOR` through `HONOLULU COUNCILMEMBER, DIST VIII`. The captured behavior therefore indicates a full-cycle export rather than a filtered export. Confirm the same behavior with `elid=94` before treating it as a permanent contract.

#### Implementation notes (added August 6, 2026)

Two behaviors matter when reimplementing the export against a plain HTTP client:

1. **Post only the export button.** The grid renders seven `type="submit"` inputs (PDF export, Clear Filter, and five pager buttons). A browser submits only the control that was clicked. Submitting all of them makes ASP.NET resolve the postback to a different control — observed result was a "Next Page" render returning `text/html` instead of the CSV.
2. **`ddlElection` is a `<select>`, not an `<input>`.** A form collector that scans only `<input>` tags silently drops it, and the portal then falls back to the `HIVIP` cookie's election. Combined with a reused session this can export a different cycle than the URL requested. Post the selected `<option>` value and verify it against the expected `elid` before trusting the response.

### Verified PDF export

The second HAR also captured a successful PDF form submission using:

```text
ctl00$cphFooter$rdgSearch$ctl00$ctl02$ctl00$ExportToPdfButton=
ctl00$cphFooter$ddlElection=87
```
...
Content-Disposition: attachment;FileName="CandidateFiling.pdf"
```

The PDF is useful for human validation, but CSV is the preferred machine-ingestion format.

### Recommended extraction method

Use the following priority order:

1. Fetch the official Candidate Reports landing page and discover the active `elid`.
2. GET the cycle-specific Candidate Filing page and retain cookies plus fresh ASP.NET hidden fields.
3. Submit the CSV export button with the current complete form state.
4. Store the raw CSV, source URL, `elid`, retrieval timestamp, response headers, and checksum.
5. Verify the expected header, cycle, record count, and representative contests.
6. Parse candidate rows and preserve raw source values.
7. Use stateful HTML pagination only if CSV export fails, is incomplete, or changes format.
8. Validate ballot-qualified candidates and final contest placement against official preview ballots.
9. Poll and version the source through filing, vacancy, withdrawal, and post-election status-update periods.

### HTML fallback requirements

An HTML fallback client must:

- maintain session cookies;
- parse fresh hidden fields after every response;
- parse actual page-specific `__doPostBack` targets;
- submit the complete current form state;
- verify the returned current-page marker and row changes;
- detect session-expiration and debug/error pages;
- deduplicate using normalized row hashes rather than page position.
...
- full `__VIEWSTATE` and `__EVENTVALIDATION` values unless needed for a temporary local replay.

Preserve request URL, method, parameter names, sanitized parameter structure, response status, response MIME type, content-disposition metadata, and timing metadata.

### Remaining candidate-source verification

Verified August 6, 2026 by direct export against the live portal:

- ✅ A current-cycle `elid=94` CSV export with no active filters returned **411 records**, matching the visible 2026 grid count. `elid=92` returned **395**, matching the 2024 grid count. Full-cycle, unfiltered export is confirmed on the current cycle.
- ✅ The 12 documented CSV columns were confirmed unchanged on `elid=94`.

Still useful but not required before beginning implementation:

- Capture a second `elid=94` CSV export with a filter active to verify whether exports consistently ignore filters.
- Capture ballot-name and legal-name filter commands only if the HTML fallback will support filtered queries.
- Capture column sorting only if sorting is needed by the fallback scraper.
- Recheck the export after the State Senate District 18 vacancy filing deadline.
...
- Proclamations define offices and broad election structure.
...
- The Contest Schedule supplies expected offices, seats, jurisdictions, and terms.
...
3. Import candidate filings and statuses from the Candidate Report CSV.
...
After result publication, attach the official election-scoped `contest_id`.
...
The index is the authoritative discovery layer. It lists election sections beginning in 1992 and links available result artifacts. As of August 5, 2026, the 2026 primary section existed before the August 8 election; its result links should be treated as pre-election placeholders until actual data is published.

### Current result-file examples

Certified 2024 General Election:

- Statewide summary text: `https://files.hawaii.gov/elections/files/results/2024/General/summary.txt`
- Statewide precinct-detail text: `https://files.hawaii.gov/elections/files/results/2024/General/media.txt`
...
| Era | Formats observed | Recommended extraction path |
|---|---|---|
| 2026 | Pre-election result links present as of August 5 | Begin ingestion only after files contain actual results |
| 2006–2024 regular primary/general | Certified summary and precinct text plus PDFs; statements of vote in many cycles | Structured text first; PDF validation |
| 2004 General | Summary and precinct text plus PDFs | Structured text first |
| 2004 Primary | Summary text plus PDFs; no precinct text listed | Summary text plus precinct PDF |
| 2010 congressional special | PDF summary and precinct detail | PDF extraction and human validation |
| 2002–2003 congressional specials | PDF summary, county reports, and precinct detail | PDF extraction and human validation |
| 2002 regular primary/general | PDF only | PDF extraction |
| 1996–2000 regular elections | Certified HTML plus PDFs | HTML first; PDF validation |
| 1996 Special Election | Certified HTML and PDFs | HTML first; PDF validation |
| 1992–1994 primary/general | Statewide summary PDFs only | PDF extraction; lower granularity |

The archive covers more than regular primary and general elections. It includes a 1996 special election and congressional special elections in November 2002, January 2003, and May 2010.

### Confirmed PDF-only special-election files

**May 22, 2010 — U.S. Representative, District I Special Vacancy Election**

- Statewide summary: `https://files.hawaii.gov/elections/files/results/2010/special/special2010-summary.pdf`
- Statewide precinct detail: `https://files.hawaii.gov/elections/files/results/2010/special/special2010-precinct.pdf`
...
- Statewide summary: `https://files.hawaii.gov/elections/files/results/2003/special/histatewide.pdf`
- Statewide precinct detail: `https://files.hawaii.gov/elections/files/results/2003/special/precinct.pdf`

The summary is one page; the precinct report is 100 pages. Use the results index for county-specific summary links.

**November 30, 2002 — U.S. Representative, District II Special Election**

- Statewide summary: `https://files.hawaii.gov/elections/files/results/2002/special/histatewide.pdf`
- Statewide precinct detail: `https://files.hawaii.gov/elections/files/results/2002/special/precinct.pdf`
...
- 1994 General: `https://files.hawaii.gov/elections/files/results/1994/general/histatewide.pdf`
- 1994 Primary: `https://files.hawaii.gov/elections/files/results/1994/primary/histatewide.pdf`
- 1992 General: `https://files.hawaii.gov/elections/files/results/1992/general/histatewide.pdf`
- 1992 Primary: `https://files.hawaii.gov/elections/files/results/1992/primary/histatewide.pdf`
...
- Organization dataset search: `https://opendata.hawaii.gov/dataset/?organization=office-of-elections`
- General Election 2012 Summary: `https://opendata.hawaii.gov/dataset/general-election-2012-summary-results`

The Office of Elections organization search returned five datasets during the research. Confirmed formats across the election datasets include CSV, RDF, JSON, and XML. Identified datasets include 2010 General Election results and 2012 primary/general summary or precinct results.
...
The previous claim that CKAN provides broad 1992–present results coverage is unsupported. A general CKAN-style registry or dataset interface should not be labeled a dedicated election API. Only confirmed election dataset resources and documented endpoints should receive an API classification.
...
Official source: `https://elections.hawaii.gov/resources/election-objections/`
...
Official source: `https://elections.hawaii.gov/resources/registration-voter-turnout-statistics/`
...
| Pipeline stage | Primary official source | Suggested identifiers and joins | Extraction and update strategy | Known gaps |
|---|---|---|---|---|
| Election calendar | Proclamation archive; current proclamation; results index | State, election date, official type, jurisdiction | Poll and version proclamations; cross-check result archive | No single normalized historical calendar download |
| Election definitions | Proclamation HTML/PDF | Proclamation date, election date, jurisdiction, office | Parse official terms and retain source version | Supplemental vacancies may arrive after the main proclamation |
| Election type | Proclamation language | Official type plus normalized type | Preserve exact Hawaii terminology | Second special elections require careful normalization |
| Offices and terms | Proclamation; Contest Schedule | Office family, seat, district, jurisdiction, term | Use schedule as baseline and proclamation as cycle authority | Schedule page may lag supplemental vacancies |
| Incumbents | Elected Officials page | Office, district, incumbent, access date | Snapshot and version | Current-state source, not historical officeholder database |
| Districts | Proclamation and GIS | District type/number, boundary vintage | Load geometry independently and attach applicable vintage | ArcGIS service metadata not fully captured |
| Precincts | GIS and result files | Precinct code, split code, precinct-split ID | Reconcile identifiers before geometry join | Crosswalk is not documented in one file |
| Contests | Proclamation, Candidate Report, preview ballots | Election, office, district, jurisdiction, party section | Create preliminary contest; validate against ballot | No confirmed pre-result stable contest ID |
| Candidates | Candidate Report CSV; HTML fallback | Source composite key and row hash | Export full cycle, hash, version, and poll through filing and vacancy deadlines | No stable candidate ID; export is undocumented |
| Filing status | Candidate Report CSV | Candidate source key plus observation time | Store as event history | Full future status vocabulary not documented |
| Ballot qualification | Candidate status and preview ballot | Candidate-to-contest and ballot-style relation | Require qualifying status and verify ballot appearance | Large PDF and possible revisions |
| Party affiliation | Candidate Report and ballot | Exact raw party plus normalized party ID | Preserve source spelling and abbreviations | Party labels vary by cycle/source |
| Ballot measures | Proclamations, question pages, results | Election, jurisdiction, text hash, result contest ID | Load definition first; attach result later | Result titles may be abbreviated |
| Results | Summary and precinct text | Election + contest ID + choice ID + precinct-split ID | Poll, hash, version, parse by schema era | Encoding and schema variation |
| Certification | Results index and certified reports | Result version plus certification evidence | Promote only from explicit official evidence | No structured certification Boolean verified |
| Recounts | Results index recount links | Election, contest, recount report | Store separate event and later result version | Reports are generally PDF |
| Election objections | Objection page and court records | Election, contest, case ID | Activate when filed; monitor disposition | Court linkage is partly manual |
| Campaign finance | SODA datasets and filing system | Committee ID, candidate, office, district, cycle | API/download polling around deadlines | No direct candidate crosswalk |
| Turnout | Turnout tables and result files | Election, county, precinct or district | Reconciliation and validation | Historical tables are aggregated |
| Historical archive | Results index | Election date/type, artifact URL | Backfill by format era | Early years and specials are PDF-heavy |
...
- Candidate Reports landing-page URL
- cycle-specific Candidate Filing URL
- `elid`
- CSV response filename
- retrieval timestamp
- CSV checksum
...
1. The current-cycle `elid=94` CSV export should be captured and compared with the visible 2026 grid count.
2. The captured 2022 CSV export ignored an active contest filter and returned all 509 records; confirm whether current-cycle exports consistently behave the same way.
3. The CSV export is undocumented and may change without notice. Keep a tested HTML fallback.
...
9. State Senate District 18 candidate data can change through September 4, 2026.
...
3. Start or reuse a verified session for the selected cycle.
4. Parse fresh ASP.NET hidden fields.
5. Submit the CSV export form action.
6. Retain the raw CSV and checksum.
7. Verify schema, cycle, visible count, exported count, and representative contests.
8. Parse candidates and store status history.
9. Fall back to stateful HTML pagination if CSV export fails or is incomplete.
10. Build elections and office definitions from proclamations.
11. Validate ballot-qualified candidates and contests against preview ballots.
12. Continue polling through supplemental vacancy deadlines and post-election status changes.
...
Hawaii should no longer be classified as a Google-Civic-only Stage 1 state. The Office of Elections provides a native candidate source with contest, party, ballot and legal names, contact fields, filing dates, and status.

The preferred candidate-ingestion path is the Candidate Report’s cycle-specific CSV export. The captured 2022 export returned 509 records and 12 columns. The export is not a documented API: it is a stateful ASP.NET form action that depends on a valid cycle page and current hidden form state. The portal’s HTML grid remains a practical fallback and a validation surface.

The HAR captures resolve the main questions from issue #156:

- page-size and numbered-page navigation are form postbacks to the same Candidate Filing URL;
- party and contest filtering commands are visible in the postback arguments;
- election-cycle switching to 2024 and 2022 was captured;
- CSV and PDF export actions were captured successfully;
- no separate candidate JSON or REST endpoint was observed.
...
- the Candidate Report CSV supplies candidate and filing-state information;
- the Candidate Report HTML grid provides fallback extraction and visual validation;
...
The remaining core work is implementation rather than source discovery: a CSV-first candidate adapter with an HTML fallback, an era-aware result-text adapter, and historical HTML/PDF fallback procedures can provide a substantially first-party Hawaii election pipeline.
