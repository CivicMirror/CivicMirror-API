...
>
> **Known open mismatch:** AL's live coverage page still reports Full Core, but this table's Stage 1 — Race Creation shows `⚠️ Untested` and the row is labeled "Results Adapter." Not resolved in this pass — needs the same live-verification treatment CT just got (or the same correction WV just got) before either the doc or the live page can be trusted for AL.
>
> **AK and WY are not Clarity states.** Both were originally swept into the generic Clarity adapter list below, but research found no historical evidence either state has ever used Clarity for ENR — the adapter choice itself was wrong, not just missing per-election config. Corrected 2026-08-11: AK tracked in issue #178, WY split out into its own issue #180. See `docs/state-research/AK/AK-Election_Research.md` and `docs/state-research/WY/WY-Election_Research.md` for the corrected native-source research.
...
| **DE, HI, ID, IN, KS, LA, ME, MS, MT, ND, NE, NH, NV, OK, RI, SD, WI** | Clarity sweep (17 states) | ✅ Available (Civic API) | ⚠️ Untested | ✅ Adapter available (Clarity) | Results Coverage Only |
| **AK** | Alaska | ✅ Available (Civic API) | ⚠️ Untested | ⚠️ Adapter mismatch — `ClarityAdapter` subclass has no verified basis; AK runs its own DOE result system (issue #178) | Results Coverage Only (adapter needs replacement) |
| **WY** | Wyoming | ✅ Available (Civic API) | ⚠️ Untested | ⚠️ Adapter mismatch — `ClarityAdapter` subclass has no verified basis; WY runs its own SOS Excel/PDF result system (issue #180) | Results Coverage Only (adapter needs replacement) |
...
- Clarity sweep states (DE, HI, ID, IN, KS, LA, ME, MS, MT, ND, NE, NH, NV, OK, RI, SD, WI, WV) — requires `results_url` set per election in Django admin. **WV corrected here 2026-07-24** — previously miscategorized as Full Core; it has no dedicated Stage 1 integration (no `integrations/wv_sos/`, no `WV_SOS` in `Race.Source`, no scheduled Stage 1 task), only the Stage 2 `results/adapters/wv.py` Clarity subclass. Election/race creation is Civic-API-driven like every other Clarity sweep state.
- Alaska (AK) — **corrected here 2026-08-11**, removed from the Clarity sweep list above. `results/adapters/ak.py` subclasses `ClarityAdapter`, but no evidence supports AK ever using Clarity for ENR (its own DOE hosts structured CSV/XML/text/CVR result files back to 1958). The adapter needs to be replaced with an AK-native one; see issue #178 and `docs/state-research/AK/AK-Election_Research.md`.
- Wyoming (WY) — **corrected here 2026-08-11**, removed from the Clarity sweep list above. `results/adapters/wy.py` subclasses `ClarityAdapter`, but no evidence supports WY ever using Clarity for ENR (its own SOS hosts an Excel-ZIP certified-results archive back to 1996). The adapter needs to be replaced with a WY-native one; see issue #180 and `docs/state-research/WY/WY-Election_Research.md`.
