...
- **Office-title qualification is required and is the one non-obvious design decision in this plan.** NM's captured election is hyper-local: verified directly against the real `Media.csv` (297KB, 1578 rows), there are **599 distinct `RaceID`s but only 229 distinct `RaceName`s** — `"Mayor"` alone is the `RaceName` for 49 different `RaceID`s across 49 different cities. If `office_title` were set to the bare `RaceName` (as MD/MO do, since their offices are inherently unique statewide titles), `results/tasks.py::_bootstrap_races_from_results` — which groups result rows by `(office_title, source_identity)` before creating `Race` rows — would silently collapse every city's "Mayor" race into one `Race` with candidates from 49 unrelated cities. `results/adapters/ct.py` already solved this exact problem for Connecticut's same-named municipal offices via `_build_office_town_map`, qualifying titles as `f"{town_name} — {base_title}"` (`ct.py:210`). This plan follows that precedent exactly: `office_title = f"{AreaNum} — {RaceName}"` (bare `RaceName` when `AreaNum` is blank — verified safe: all 8 blank-`AreaNum` rows in the real file are county-level ballot measures whose `RaceName` is already a long, globally-unique question text). Every row also carries `raw={"contest_code": RaceID, "party_code": PartyCode}` so `results/tasks.py`'s `_row_source_identity`/`_race_source_identity` can do exact per-race identity matching as a second line of defense against the same collision risk.
