...
### 2. `backend/results/adapters/nj.py` (Stage 2)
...
- For each county URL: fetch `current_ver.txt` (skip that county gracefully — log and continue, not abort — if the fetch fails or the county has no ID), fetch `summary.json`, call the inherited `self._parse_contests()` to get that county's `ResultRow`s tagged with the county name (via a new field or `raw` metadata, so per-county provenance isn't lost even though rows get summed).
- **Aggregation**: sum `vote_count` by `(office_title, candidate_name)` across all counties' rows for statewide races (offices that could plausibly appear in every county — US Senate, Governor, President). District-scoped races (Congressional, State Senate/House) only draw from the counties that actually contain that district — no fabricated cross-county summing for races a county doesn't have.
...
- Unit tests for cross-county aggregation (sum by office+candidate across synthetic multi-county `summary.json` fixtures; verify district races don't get counties they shouldn't).
