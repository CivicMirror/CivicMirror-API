...
3. Run this command (``--dry-run`` first to preview; it writes an audit file in
   both modes so collision-group members can be inspected before applying).
4. Resume schedulers.
...
def _process_collision_group(new_key, race_pks, stats, audit, apply_changes):
    ...
    with transaction.atomic():
        ...
        if not apply_changes:
            for loser in losers:
                group_record["losers"].append({
                    ...
                    "loser_office_title": loser.office_title,
        ...
        try:
            ...
            Race.objects.filter(pk__in=[loser.pk for loser in losers]).delete()
